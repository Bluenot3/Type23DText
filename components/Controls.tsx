    ...
    <>
      <div className={`absolute bottom-4 right-4 z-10 text-white transition-all duration-300 ease-in-out ${isOpen ? 'w-64 md:w-80 p-2 bg-black/40 backdrop-blur-md rounded-lg shadow-2xl border border-white/10 max-h-[calc(100vh-4rem)] overflow-y-auto visible opacity-100' : 'w-0 h-0 overflow-hidden invisible opacity-0'}`}>
        <div className="space-y-2">
            <Section title="Text & Font" defaultOpen={true}>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Display Text</label>
                  <input type="text" value={props.text} onChange={(e) => props.setText(e.target.value)} maxLength={8} className="w-full bg-gray-900/50 text-white placeholder-gray-500 border border-white/20 rounded-md px-3 py-2 text-sm focus:ring-sky-500 focus:border-sky-500" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Font</label>
                  <select value={props.font} onChange={(e) => props.setFont(e.target.value)} className="w-full bg-gray-900/50 text-white border border-white/20 rounded-md px-3 py-2 text-sm focus:ring-sky-500 focus:border-sky-500">
            ...
            <Section title="Geometry">
              <ControlSlider label="Depth" value={props.textHeight} onChange={e => props.setTextHeight(parseFloat(e.target.value))} min={0.1} max={5} step={0.05} />
              <ControlSlider label="Bevel Thickness" value={props.bevelThickness} onChange={e => props.setBevelThickness(parseFloat(e.target.value))} min={0} max={0.5} step={0.01} />
              <ControlSlider label="Bevel Size" value={props.bevelSize} onChange={e => props.setBevelSize(parseFloat(e.target.value))} min={0} max={0.5} step={0.01} />
            </Section>
            
            <Section title="Animation">
                <label className="block text-sm font-medium text-gray-300">Style</label>
                <select value={props.animation} onChange={(e) => props.setAnimation(e.target.value)} className="w-full bg-gray-900/50 text-white border border-white/20 rounded-md px-3 py-2 text-sm focus:ring-sky-500 focus:border-sky-500">
            ...
            <Section title="Material">
                 <div>
                  ...
                  <select value={props.materialPreset} onChange={(e) => props.setMaterialPreset(e.target.value)} className="w-full bg-gray-900/50 text-white border border-white/20 rounded-md px-3 py-2 text-sm focus:ring-sky-500 focus:border-sky-500">
                    {Object.keys(MATERIAL_PRESETS).map((name) => (<option key={name} value={name}>{name}</option>))}
                  </select>
                </div>
                <div className="flex justify-between items-center">
                    <label className="text-sm font-medium text-gray-300">Color</label>
                    <input type="color" value={props.color} onChange={e => props.setColor(e.target.value)} className="w-8 h-8 p-0 border-none rounded bg-transparent" />
                </div>
                <ControlSlider label="Roughness" value={props.roughness} onChange={(e) => props.setRoughness(parseFloat(e.target.value))} min={0} max={1} step={0.01} />
                <ControlSlider label="Metalness" value={props.metalness} onChange={(e) => props.setMetalness(parseFloat(e.target.value))} min={0} max={1} step={0.01} />
            ...
            <Section title="Texture Generation">
                <div>
                  ...
                  <input type="text" value={props.cascadingText} onChange={(e) => props.setCascadingText(e.target.value)} maxLength={30} className="w-full bg-gray-900/50 text-white placeholder-gray-500 border border-white/20 rounded-md px-3 py-2 text-sm focus:ring-sky-500 focus:border-sky-500" />
                </div>
                 <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Glyph Preset</label>
                  <select value={props.textureGlyphPreset} onChange={(e) => props.setTextureGlyphPreset(e.target.value)} className="w-full bg-gray-900/50 text-white border border-white/20 rounded-md px-3 py-2 text-sm focus:ring-sky-500 focus:border-sky-500">
                ...
                {props.textureGlyphPreset === 'Custom' && (
                  <div>
                    ...
                    <input type="text" value={props.textureGlyphSet} onChange={(e) => props.setTextureGlyphSet(e.target.value)} className="w-full bg-gray-900/50 text-white placeholder-gray-500 border border-white/20 rounded-md px-3 py-2 text-sm focus:ring-sky-500 focus:border-sky-500" />
                  </div>
                )}
                 <ControlSlider label="Texture Density" value={props.textDensity} onChange={(e) => props.setTextDensity(parseFloat(e.target.value))} min={0.05} max={1.0} step={0.01} />
                 <ControlSlider label="Fall Speed" value={props.textureFallSpeed} onChange={(e) => props.setTextureFallSpeed(parseFloat(e.target.value))} min={0.1} max={5} step={0.1} />
            ...
            <Section title="Lighting">
              <div className="flex justify-between items-center">
                  ...
                  <input type="color" value={props.light1Color} onChange={e => props.setLight1Color(e.target.value)} className="w-8 h-8 p-0 border-none rounded bg-transparent" />
              </div>
              <ControlSlider label="Primary Intensity" value={props.light1Intensity} onChange={e => props.setLight1Intensity(parseFloat(e.target.value))} min={0} max={5} step={0.1} />
              <div className="flex justify-between items-center">
                  <label className="text-sm font-medium text-gray-300">Secondary Color</label>
                  <input type="color" value={props.light2Color} onChange={e => props.setLight2Color(e.target.value)} className="w-8 h-8 p-0 border-none rounded bg-transparent" />
              </div>
              <ControlSlider label="Secondary Intensity" value={props.light2Intensity} onChange={e => props.setLight2Intensity(parseFloat(e.target.value))} min={0} max={5} step={0.1} />
            </Section>
            
            <Section title="Environment">
                <label className="block text-sm font-medium text-gray-300">Preset</label>
                <select value={props.envPreset} onChange={(e) => props.setEnvPreset(e.target.value)} className="w-full bg-gray-900/50 text-white border border-white/20 rounded-md px-3 py-2 text-sm focus:ring-sky-500 focus:border-sky-500">
                  {ENV_LIST.map((name) => (<option key={name} value={name}>{name.charAt(0).toUpperCase() + name.slice(1)}</option>))}
                </select>
            </Section>
            
            <Section title="Post-Processing">
                <ControlSlider label="Bloom" value={props.bloomIntensity} onChange={(e) => props.setBloomIntensity(parseFloat(e.target.value))} min={0} max={3} step={0.01} />
                <ControlSlider label="Chromatic Aberration" value={props.chromaticAberrationOffset} onChange={(e) => props.setChromaticAberrationOffset(parseFloat(e.target.value))} min={0} max={0.01} step={0.0001} displayFactor={1000} />
                <ControlSlider label="Vignette" value={props.vignetteDarkness} onChange={(e) => props.setVignetteDarkness(parseFloat(e.target.value))} min={0} max={2} step={0.01} />
                <ControlSlider label="God Rays" value={props.godRaysIntensity} onChange={(e) => props.setGodRaysIntensity(parseFloat(e.target.value))} min={0} max={1} step={0.01} />
                <ControlSlider label="Noise" value={props.noiseIntensity} onChange={(e) => props.setNoiseIntensity(parseFloat(e.target.value))} min={0} max={0.2} step={0.001} />
                <ControlSlider label="Depth of Field" value={props.dofIntensity} onChange={(e) => props.setDofIntensity(parseFloat(e.target.value))} min={0} max={1} step={0.01} />
